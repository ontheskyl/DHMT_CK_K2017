...
namespace Lab04
{
    class Background
    {
        ...
        public void Draw(OpenGLControl openGLControl, int size)
        {
            ...
            gl.Color(1f, 0, 0, 0); // Chọn màu Đỏ cho trục x
            gl.Vertex(-size, 0, 0); gl.Vertex(size, 0, 0);

            gl.Color(0f, 1f, 0, 0); // Chọn màu Xanh lục cho trục y
            gl.Vertex(0, -size, 0); gl.Vertex(0, size, 0);

            gl.Color(0f, 0, 1f, 0); // Chọn màu Xanh dương cho trục z
            ...
            gl.Begin(OpenGL.GL_LINES);
            gl.LineWidth(0.1f);
            gl.Color(160.0/255.0, 160.0/255.0, 160.0/255.0, 0);
            //gl.Color(0, 0, 0, 0);

            for (float i = -size * 2; i <= size * 2; i += 0.5f)
                if (i != 0)
                {
                    gl.Vertex(-size * 2, 0, i);
                    gl.Vertex(size * 2, 0, i);
                    gl.Vertex(i, 0, -size * 2);
                    gl.Vertex(i, 0, size * 2);
                }
            gl.End();
        }
